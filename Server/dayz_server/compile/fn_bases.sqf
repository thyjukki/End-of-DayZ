...
private ["_clutter","_index","_lootMaxRadius2","_itemType","_position","_item","_nearby","_basePos","_baseClass","_lootMinRadius","_lootMaxRadius","_randomLoot","_guaranteedLoot","_lootTable","_itemTypes","_weights","_cntWeights","_i","_timeExit"];
	...
	_i = 0;
		while {(_i < ((round(random _randomLoot)) + _guaranteedLoot))} do { //timeout
			//create loot
			//diag_log (_rndloot);
			_itemTypes = [] + getArray (missionConfigFile >> "CfgBuildingLoot" >> "InfectedCamps" >> "lootType");
				...
				switch (_iClass) do {
					default {
						...
						//while {_qty < _max} do {
							...
							//diag_log ("fn_bases iCLass: "+str(_iClass));
							...
							//diag_log ("fn_bases CanType: "+str(_canType));
							...
							if (_tQty > 0) then {
								if (!(_canType in _uniq)) then {
									//diag_log(format["fn_bases default (%1)",_canType]);
						...
						if (_iItem != "") then {
							//diag_log(format["fn_bases default (%1)",_iItem]);
							_item addMagazineCargoGlobal [_iItem,1];
							//diag_log ("Unknown");
						};
					};
					case "weapon": {
						//diag_log(format["fn_bases magazine (%1)",_iItem]);
						...
						if ((count _mags) > 0) then {
							if (!(_iItem in MeleeWeapons)) then {
								...
								while {_qty < _max} do {
									...
									//diag_log(format["fn_bases ammo (%1) spawning to %3",_iItem, _magtoadd]);
					...
					case "tool": {
						//diag_log(format["fn_bases tool (%1)",_iItem]);
						_item addWeaponCargoGlobal [_iItem,round(random(1) + 1)];
					};
					case "backpack": {
						//diag_log(format["fn_bases backpack (%1)",_iItem]);
