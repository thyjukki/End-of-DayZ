...
switch (_iClass) do {
	default {
		...
		//while {_qty < _max} do {
			...
			//diag_log format ["%1, %2", _iClass, _canType];
			...
			if (_tQty > 0) then {
				if (!(_canType in _uniq)) then {
					//diag_log(format["server_SpawnLoot default (%1)",_canType]);
		...
		//};
		if (_iItem != "") then {
			//diag_log(format["server_SpawnLoot default (%1)",_iItem]);
	...
	case "weapon": {
		...
		//diag_log(format["server_SpawnLoot weapon (%1)",_iItem]);
		_item = createVehicle ["WeaponHolder", _iPos, [], _radius, "CAN_COLLIDE"];
		...
		if ((count _mags) > 0) then {
			if (!(_iItem in MeleeWeapons)) then {
				...
				while {_qty < _max} do {
					...
					//diag_log(format["server_SpawnLoot ammo (%1) spawning to %3",_iItem, _magtoadd]);
	...
	*/
	case "magazine": {
		...
		diag_log(format["server_SpawnLoot magazine (%1)",_iItem]);
		_item = createVehicle ["WeaponHolder", _iPos, [], _radius, "CAN_COLLIDE"];
		_item addMagazineCargoGlobal [_iItem,1];
	};
	case "object": {
		//Item is one magazine
		diag_log(format["server_SpawnLoot object (%1)",_iItem]);
		_item = createVehicle [_iItem, _iPos, [], _radius, "CAN_COLLIDE"];
	};
	case "backpack": {
		diag_log(format["server_SpawnLoot backpack (%1)",_iItem]);
