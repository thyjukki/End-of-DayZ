...
//Has some kind of target
	...
	_isDetained = _cursorTarget getVariable ["Detain",0];
	_isEscorted = _cursorTarget getVariable ["Escort",0];
	_isFuel = false;
	_isKerosene = false;
	...
	if(_isMan && !_isZombie && !_isAnimal && _isAlive && _hasZiptie && _isDetained == 0) then 
	{
		if (s_player_ziptie < 0) then 
		{
			s_player_ziptie = player addaction ["Handcuff", "\z\addons\dayz_code\actions\handcuff.sqf","",100,false,true,"", ""];
		};
	} 
	else 
	{
		player removeAction s_player_ziptie;
		s_player_ziptie = -1;
	};

	if(_isMan && !_isZombie && !_isAnimal && _isAlive && _isDetained != 0) then 
	{
		if (s_player_release < 0) then 
		{
			s_player_release = player addaction ["Release", "\z\addons\dayz_code\actions\release.sqf","",100,false,true,"", ""];
		};
	} 
	else 
	{
	...
	if(_isMan && !_isZombie && !_isAnimal && _isAlive && _isDetained != 0 && _isEscorted == 0) then 
	{
		...
		{
			s_player_escort = player addaction ["Escort", "\z\addons\dayz_code\actions\escort.sqf","",100,false,true,"", ""];
		};
	} 
	else 
	{
	...
	if(_isMan && !_isZombie && !_isAnimal && _isAlive && _isDetained != 0 && _isEscorted != 0) then 
	{
		if (s_player_stop_escort < 0) then 
		{
			s_player_stop_escort = player addaction ["Stop Escort", "\z\addons\dayz_code\actions\stop_escort.sqf","",100,false,true,"", ""];
		};
	} 
	else 
	{
		player removeAction s_player_stop_escort;
		s_player_stop_escort = -1;
	};

	if (!_isAlive and !_isZombie and !_isAnimal and _hasShovel and _isMan and _canDo) then {
        if (s_player_bury_human < 0) then {
            s_player_bury_human = player addAction [format["Bury Body"], "\z\addons\dayz_code\actions\bury_human.sqf",cursorTarget, 0, false, true, "", ""];
        }
    } else {
	...
	if (_isMan && !_isZombie && primaryweapon _cursorTarget == "" and _isAlive and !_isZombie and !_isAnimal) then
	{
		if (s_player_friskbody < 0) then
		{
			s_player_friskbody = player addAction [localize "str_actions_frisk", "\z\addons\dayz_code\actions\frisk.sqf",_cursorTarget, 0, false, true, "",""];
		};
	}
	else
	{
