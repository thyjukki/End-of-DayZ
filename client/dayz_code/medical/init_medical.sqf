...
while {1 == 1} do {
	...
	//blood pools
	if (count r_player_bloodpools > 0) then { //Are we already removing puddles, or should we start to
		_obj = r_player_bloodpools select 0; //Get first puddle in the array, the oldest one
		_ticktime = _obj getVariable ["bloodpools",false];
		if (diag_ticktime - _ticktime > 60) then {
			r_player_bloodpools = r_player_bloodpools - [_obj]; //Remove it from the array
			
			PVDZ_obj_Delete = [_obj,player];
			publicVariableServer "PVDZ_obj_Delete";
			
			//_recycleArray = (count r_player_bloodpools > 0); //Check if we need to keep removing puddles
		};
	};
	...
	if ((r_player_bloodregen < 0.1) or (r_player_blood == 12000)) then {
	...
	[] spawn {
		if (!r_player_unconscious) then {
			if (((r_player_blood/r_player_bloodTotal) < 0.35)) then {
				...
				1 fadeSound 1; //restoring sound to normal if conscious
			};
		};
	};
